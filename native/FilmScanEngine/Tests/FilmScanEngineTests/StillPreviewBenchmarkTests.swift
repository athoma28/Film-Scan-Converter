...
struct StillPreviewBenchmarkTests {
  ...
  @Test("Scene display GPU renderer matches the authoritative CPU parameter grid")
  func sceneDisplayRendererMatchesCPU() {
    let scene: [Double] = [
      0, 0.000_001, 0.01,
      0.05, 0.1, 0.25,
      0.5, 0.75, 1,
    ...
    let parameterGrid = [
      ...
      DisplayRenderingParameters(
        exposureEV: 1.25,
        whiteBalance: BGRChannelValues(blue: 0.7, green: 1.0, red: 1.4),
        toneMap: .linear,
        maximumSceneGain: 8
      ),
      DisplayRenderingParameters(
        exposureEV: -2,
        whiteBalance: BGRChannelValues(blue: 1.8, green: 0.9, red: 0.6),
        toneMap: .reinhard,
        maximumSceneGain: 16
      ),
      DisplayRenderingParameters(
        exposureEV: 5,
        whiteBalance: BGRChannelValues(blue: 4, green: 2, red: 8),
        toneMap: .reinhard,
        maximumSceneGain: 3
      ),
    ]

    guard let renderer = SceneDisplayRenderer(sceneLinear: scene, width: 5, height: 1) else {
      #expect(Bool(false), "Could not create scene display renderer")
      return
    }

    for parameters in parameterGrid {
      guard let gpu = renderer.render(parameters: parameters) else {
        #expect(Bool(false), "Scene display GPU render failed")
        continue
      }
      let cpu = FilmNegativeProcessing.renderDisplay(
        sceneLinear: scene,
      ...
      for index in cpu.indices {
        #expect(
          abs(gpu[index] - cpu[index]) <= 0.000_002,
          "display[\(index)] GPU=\(gpu[index]) CPU=\(cpu[index])"
        )
      }
    }
  }

  @Test("Scene display GPU renderer preserves finite bounds for invalid scene samples")
  func sceneDisplayRendererSanitizesInput() {
    let scene: [Double] = [.nan, .infinity, -.infinity, -1, 1, 10]
    let parameters = DisplayRenderingParameters(toneMap: .reinhard)

    guard
      let renderer = SceneDisplayRenderer(sceneLinear: scene, width: 2, height: 1),
      let gpu = renderer.render(parameters: parameters)
    else {
      #expect(Bool(false), "Could not render invalid scene samples")
      return
    }

    #expect(gpu.allSatisfy { $0.isFinite && $0 >= 0 && $0 <= 1 })
    #expect(gpu[0] == 0)
